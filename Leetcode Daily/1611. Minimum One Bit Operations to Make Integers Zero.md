...
````
Explanation:  
Binary `6` (110) → requires 4 bit operations to reach `000`.
...
3. This solution applies that principle in reverse — computing the cumulative XOR of `n` and its right shifts.
...
* **Time Complexity:** `O(log n)` — one iteration per bit of `n`.
...
* `n = 1`: returns `1`.
* Large values of `n` (up to 2³¹ or beyond): handled efficiently since loop runs per bit.
* Powers of two (e.g., `n = 8`) → ensure correctness for binary boundaries.
