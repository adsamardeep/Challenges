# 🏷 Find all people who learn a secret through timed meetings
...
* Time complexity: **O(m log m + m α(n))**, where `m` is the number of meetings (sorting dominates; Union-Find operations are nearly constant).
* Space complexity: **O(n)** for the parent array.

## 🧪 Edge Cases

* No meetings → only persons `0` and `firstPerson` know the secret.
* `firstPerson = 0` → secret starts with only one person.
* Multiple meetings at the same time → handled together to avoid incorrect propagation.
* Disconnected meeting groups → correctly isolated by resetting parents.
* All people connected through time-ordered meetings → everyone learns the secret.
