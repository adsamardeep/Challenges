...
* Window 0: [1,2,2,3] → freqs {(2:2),(3:1),(1:1)} → top2 = (2 with f=2) and (3 with f=1) → sum = 2*2 + 3*1 = 7
* Window 1: [2,2,3,3] → freqs {(2:2),(3:2)} → tie on frequency → pick larger number first → top2 = (3,f=2) and (2,f=2) → sum = 3*2 + 2*2 = 10
* Window 2: [2,3,3,3] → freqs {(3:3),(2:1)} → top2 = (3,f=3) and (2,f=1) → sum = 3*3 + 2*1 = 11
...
* For each window we sort `V` items: `O(V log V)` per window. There are `O(n)` windows → total time `O(n * V log V)`.
* Space is `O(V)` for the frequency table plus `O(n)` for the output.

If `V` is small (like ≤ 100), this is practical. If `V` is large (values up to 10⁵ or unbounded), prefer:
...
* `k > len(nums)` → returns `[]`.
* `x = 0` → result is all zeros (no numbers to pick).
