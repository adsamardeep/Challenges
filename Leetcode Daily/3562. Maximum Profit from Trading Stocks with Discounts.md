# 🏷 Tree-based knapsack optimization with discounts and capacity constraint
...
4. Merge children’s DP arrays using a knapsack-style convolution (`mg`), respecting the capacity limit `b`.
5. After merging all children, incorporate the current node’s profit and cost using another transition (`mk`), producing the two DP states for node `u`.
...
* `b = 0`: no capacity available → profit is `0`.
