...
**Query #1**
    ...
    FilteredData AS (
        SELECT id, value
        FROM CombinedData
        WHERE (value BETWEEN 32 AND 34) 
           OR (value IN (39, 40, 41, 44, 45, 46, 58, 59, 63))
           OR (value BETWEEN 65 AND 90) 
           OR (value BETWEEN 97 AND 122)
    )
    SELECT GROUP_CONCAT(CHAR(value) ORDER BY id SEPARATOR '') AS decoded_message
    FROM FilteredData;
...
[View on DB Fiddle](https://www.db-fiddle.com/f/7APs2BhEYvFSLWQtnKBdcC/0)